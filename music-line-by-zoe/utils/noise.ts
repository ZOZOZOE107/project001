/**
 * A simple implementation of Perlin Noise for 2D.
...
// Export 2D noise helper similar to p5.noise(x, y)
// p5 noise returns 0-1. This implementation typically returns -1 to 1, but we can normalize if needed.
// However, the original code uses (noise() - 0.5), implying it expects 0-1 range.
export const noise = (x: number, y: number): number => {
    // Basic perlin output is roughly -1 to 1.
    // Map to 0 to 1
    const n = noiseInstance.noise(x, y, 0);
    return (n + 1) / 2;
};
