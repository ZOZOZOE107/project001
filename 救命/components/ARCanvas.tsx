...
const ARCanvas: React.FC<ARCanvasProps> = ({ settings, onSettingsChange, textQueue, onTextConsumed, isCameraOn, clearTrigger, rewindTrigger, zoomLevel = 1, verticalOffset = 0 }) => {
  ...
  return (
    <div 
        ...
        // Change: Added rounded-xl for subtle rounded corners on mobile, removed on desktop.
        // Change: Ensure desktop still uses full height (top-0 h-full).
        className={`absolute left-0 w-full md:top-0 md:h-full md:bottom-auto top-[60px] bottom-[60px] overflow-hidden transition-colors duration-500 rounded-xl md:rounded-none ${isCameraOn ? 'bg-black' : 'bg-white'}`}
    >
